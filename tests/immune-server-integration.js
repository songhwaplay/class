...
async function joinStudent(name, clients) {
    ...
    client.send({ type: "JOIN_ROOM", gameId: "immune", roomCode: "7315", name });
...
async function run() {
    ...
    try {
        ...
        host.send({ type: "CREATE_ROOM", gameId: "immune", roomCode: "7315", name: "교사" });
        await host.waitFor((message) => message.type === "ROOM_CREATED", "학급 생성");
        await host.waitFor((message) => message.type === "IMMUNE_STATE" && message.state.phase === "lobby", "초기 상태");
        ...
        const ready = await second.client.waitFor((message) => message.type === "IMMUNE_STATE" && message.state.participants.length === 2, "2명 대기 상태");
        assert.deepEqual(ready.state.participants.map((player) => player.name).sort(), ["바다", "하늘"]);

        host.send({ type: "IMMUNE_ACTION", action: "START" });
        const hostStart = await host.waitFor((message) => message.type === "IMMUNE_STATE" && message.state.phase === "running", "교사 출발 상태");
        const firstStart = await first.client.waitFor((message) => message.type === "IMMUNE_STATE" && message.state.phase === "running", "첫 학생 출발 상태");
        const secondStart = await second.client.waitFor((message) => message.type === "IMMUNE_STATE" && message.state.phase === "running", "둘째 학생 출발 상태");
        ...
        late.send({ type: "JOIN_ROOM", gameId: "immune", roomCode: "7315", name: "노을" });
        const lateError = await late.waitFor((message) => message.type === "ERROR", "진행 중 입장 거절");
        assert.match(lateError.message, /이미 출발한/);

        first.client.send({ type: "IMMUNE_ACTION", action: "SUBMIT", sessionId: firstStart.state.sessionId, score: 8 });
        const firstResult = await first.client.waitFor((message) => message.type === "IMMUNE_STATE" && message.state.rankings.length === 1, "첫 완주 결과");
        assert.equal(firstResult.state.rankings[0].name, "하늘");
        assert.equal(firstResult.state.rankings[0].score, 8);

        second.client.send({ type: "IMMUNE_ACTION", action: "SUBMIT", sessionId: secondStart.state.sessionId, score: 10 });
        const finalState = await second.client.waitFor((message) => message.type === "IMMUNE_STATE" && message.state.phase === "ended", "최종 순위");
        assert.deepEqual(finalState.state.rankings.map((entry) => [entry.rank, entry.name, entry.score]), [[1, "바다", 10], [2, "하늘", 8]]);

        host.send({ type: "IMMUNE_ACTION", action: "RESET" });
        const reset = await host.waitFor((message) => message.type === "IMMUNE_STATE" && message.state.phase === "lobby" && message.state.participants.length === 2, "새 탐험 준비");
        assert.equal(reset.state.rankings.length, 0);

        console.log("immune-server-integration: synchronized start, ranking, late-join guard and reset ok");
    } finally {
