...
async function joinStudent(name, clients) {
    ...
    client.send({ type: "JOIN_ROOM", gameId: "circulation", roomCode: "6842", name });
...
async function run() {
    ...
    try {
        ...
        host.send({ type: "CREATE_ROOM", gameId: "circulation", roomCode: "6842", name: "교사" });
        await host.waitFor((message) => message.type === "ROOM_CREATED", "학급 생성");
        await host.waitFor((message) => message.type === "CIRCULATION_STATE" && message.state.phase === "lobby", "초기 탐험 상태");
        ...
        const twoStudents = await second.client.waitFor((message) => message.type === "CIRCULATION_STATE" && message.state.participants.length === 2, "2명 탐험 상태");
        assert.deepEqual(twoStudents.state.participants.map((player) => player.name).sort(), ["바다", "하늘"]);

        host.send({ type: "CIRCULATION_ACTION", action: "START" });
        const hostStart = await host.waitFor((message) => message.type === "CIRCULATION_STATE" && message.state.phase === "running", "교사 출발 상태");
        const firstStart = await first.client.waitFor((message) => message.type === "CIRCULATION_STATE" && message.state.phase === "running", "첫 학생 출발 상태");
        const secondStart = await second.client.waitFor((message) => message.type === "CIRCULATION_STATE" && message.state.phase === "running", "둘째 학생 출발 상태");
        ...
        late.send({ type: "JOIN_ROOM", gameId: "circulation", roomCode: "6842", name: "노을" });
        ...
        host.send({ type: "CIRCULATION_ACTION", action: "RESET" });
        const resetState = await host.waitFor((message) => message.type === "CIRCULATION_STATE" && message.state.phase === "lobby" && message.state.participants.length === 2, "새 탐험 준비");
        assert.equal(resetState.state.participants.length, 2, "연결된 학생은 다음 탐험 대기실에 남아야 합니다.");

        console.log("circulation-server-integration: synchronized start, server ranking, late-join guard and reset ok");
    } finally {
