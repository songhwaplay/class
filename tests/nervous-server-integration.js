...
async function joinStudent(name, clients) {
    ...
    client.send({ type: "JOIN_ROOM", gameId: "nervous", roomCode: "6428", name });
...
async function run() {
    ...
    try {
        ...
        host.send({ type: "CREATE_ROOM", gameId: "nervous", roomCode: "6428", name: "교사" });
        await host.waitFor((message) => message.type === "ROOM_CREATED", "학급 생성");
        await host.waitFor((message) => message.type === "NERVOUS_STATE" && message.state.phase === "lobby", "초기 상태");
        ...
        const ready = await second.client.waitFor((message) => message.type === "NERVOUS_STATE" && message.state.participants.length === 2, "2명 대기 상태");
        assert.deepEqual(ready.state.participants.map((player) => player.name).sort(), ["바다", "하늘"]);

        host.send({ type: "NERVOUS_ACTION", action: "START" });
        const hostStart = await host.waitFor((message) => message.type === "NERVOUS_STATE" && message.state.phase === "running", "교사 출발 상태");
        const firstStart = await first.client.waitFor((message) => message.type === "NERVOUS_STATE" && message.state.phase === "running", "첫 학생 출발 상태");
        const secondStart = await second.client.waitFor((message) => message.type === "NERVOUS_STATE" && message.state.phase === "running", "둘째 학생 출발 상태");
        ...
        late.send({ type: "JOIN_ROOM", gameId: "nervous", roomCode: "6428", name: "노을" });
        ...
        host.send({ type: "NERVOUS_ACTION", action: "RESET" });
        const reset = await host.waitFor((message) => message.type === "NERVOUS_STATE" && message.state.phase === "lobby" && message.state.participants.length === 2, "새 탐험 준비");
        assert.equal(reset.state.rankings.length, 0);

        console.log("nervous-server-integration: synchronized start, ranking, late-join guard and reset ok");
    } finally {
