...
(async()=>{
 ...
 const entered=await ack(student,'enterCity',{placeId:'lisbon'});assert.equal(entered.ok,true,entered.error);assert.equal(entered.self.mode,'city');assert.equal(entered.self.transition,null);assert.match(entered.self.currentCityImage,/city_000\.png\?v=53$/);
