...
(async()=>{
  ...
  assert.equal(completed.you.finishRank,1);assert.equal(completed.you.speedBoostMultiplier,2);assert.equal(completed.you.missionCompleted,true);
  ...
  const teacherSnap=await once(teacher,'teacherSnapshot',s=>s.players?.some(p=>p.name==='완료자'&&p.finishRank===1&&p.speedBoostMultiplier===2));
  assert.ok(teacherSnap.players.some(p=>p.name==='완료자'&&p.missionCompleted));
  console.log(JSON.stringify({ok:true,rank:1,speedBoost:2,movedPixels:Math.round(distance(origin,moved.you))}));
