...
assert.equal(type(10,25).type,'desert');assert.equal(type(-60,-5).type,'forest');assert.equal(type(85,30).type,'highMountain');assert.equal(type(85,30).passable,false);assert.equal(type(-9.1,38.7).passable,true);
assert.deepEqual(T.SPEED,{sea:1,plain:.88,coast:.76,river:.58,forest:.54,desert:.43,mountain:.30,highMountain:0});
