...
function FractionStack({ numerator, denominator, className = "" }: { numerator: ReactNode; denominator: ReactNode; className?: string }) {
  return (
    <span className={`grade-three-fraction-stack ${className}`}>
    ...
    return (
      <div className={`multiplication-question grade-three-fraction-question value${resultClass(problem)}`} data-testid="grade-three-fraction-question" key={problem.id}>
        ...
        {answerSheet
          ...
          : <input className="grade-three-fraction-value-input" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} value={answer.value} onChange={(event) => updateAnswer(problem.id, "value", event.target.value)} aria-label={`${index + 1}번 답`} />}
  ...
  function renderRelationProblem(problem: RelationProblem, index: number, answerSheet: boolean) {
    ...
    const fraction = answerSheet
      ...
      : <FractionStack
          ...
          numerator={<input className="grade-three-fraction-part-input" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={1} value={answer.numerator} onChange={(event) => updateAnswer(problem.id, "numerator", event.target.value)} aria-label={`${index + 1}번 분자 답`} />}
          denominator={<input className="grade-three-fraction-part-input" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={1} value={answer.denominator} onChange={(event) => updateAnswer(problem.id, "denominator", event.target.value)} aria-label={`${index + 1}번 분모 답`} />}
        />;
    return (
