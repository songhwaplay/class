...
import { createGradeFiveFractionOneSet } from "../../../lib/grade-five-fraction-one";
import type {
  FractionOneOperand,
  GradeFiveFractionOneProblem,
  MixedFractionAnswer,
} from "../../../lib/grade-five-fraction-one";
...
function Operand({ value }: { value: FractionOneOperand }) {
  if (value.kind === "natural") {
    return <strong className="grade-five-fraction-one-natural">{value.value}</strong>;
  }
  return (
    <span className="grade-five-fraction-one-value">
...
export default function GradeFiveFractionOnePage() {
  ...
  const problemSet = useMemo(() => createGradeFiveFractionOneSet(seed), [seed]);
  ...
  function isAnswerCorrect(problem: GradeFiveFractionOneProblem) {
    ...
    const fractionCorrect = problem.answer.numerator === 0
      ...
      : written.numerator === String(problem.answer.numerator)
        && written.denominator === String(problem.answer.denominator);
  ...
  function renderAnswer(problem: GradeFiveFractionOneProblem, answerSheet: boolean) {
    if (answerSheet) {
      return <span className="grade-five-fraction-one-static-answer"><MixedValue answer={problem.answer} /></span>;
    }
    const answer = answers[problem.id] ?? { whole: "", numerator: "", denominator: "" };
    return (
      <span className="grade-five-fraction-one-answer">
        <input className="grade-five-fraction-one-whole-input" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={2} value={answer.whole} onChange={(event) => updateAnswer(problem.id, "whole", event.target.value)} data-fraction-answer-input="true" onKeyDown={moveBetweenFractionAnswerInputs} aria-label={`${problem.id} 자연수 부분`} />
        <FractionStack
          className="answer"
          inputOrder
          numerator={<input className="grade-five-fraction-one-part-input numerator" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={4} value={answer.numerator} onChange={(event) => updateAnswer(problem.id, "numerator", event.target.value)} data-fraction-answer-input="true" onKeyDown={moveBetweenFractionAnswerInputs} aria-label={`${problem.id} 분자`} />}
          denominator={<input className="grade-five-fraction-one-part-input denominator" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={4} value={answer.denominator} onChange={(event) => updateAnswer(problem.id, "denominator", event.target.value)} data-fraction-answer-input="true" onKeyDown={moveBetweenFractionAnswerInputs} aria-label={`${problem.id} 분모`} />}
        />
      </span>
    );
  }

  function renderProblem(problem: GradeFiveFractionOneProblem, index: number, answerSheet: boolean) {
    const graded = problem.id in results;
    const isCorrect = results[problem.id] === true;
    return (
      <div className={`multiplication-question grade-five-fraction-one-question${graded ? isCorrect ? " is-correct" : " is-wrong" : ""}`} data-testid="grade-five-fraction-one-question" key={problem.id}>
        <span className="grade-five-fraction-one-index">{index + 1}</span>
        <div className="grade-five-fraction-one-expression">
          {problem.operands.map((operand, operandIndex) => (
            <span className="grade-five-fraction-one-term" key={operandIndex}>
              {operandIndex > 0 && <span className="grade-five-fraction-one-operator">{problem.operator}</span>}
