...
export default function GradeFourAngleEstimationPage() {
  ...
  const problems = useMemo(() => createAngleEstimationSet(seed), [seed]);
  ...
  function moveOnEnter(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    const inputs = Array.from(document.querySelectorAll<HTMLInputElement>("[data-angle-answer]"));
  ...
  function checkAll() {
    setResults(Object.fromEntries(problems.map((problem) => [
      problem.id,
      answers[problem.id] === String(problem.angle),
    ])));
  }
  ...
  function renderProblem(problem: AngleEstimationProblem, index: number, answerSheet: boolean) {
    const graded = problem.id in results;
    const isCorrect = results[problem.id] === true;
    return (
      <div className={`angle-estimation-question${graded ? isCorrect ? " is-correct" : " is-wrong" : ""}`} data-testid="angle-estimation-question" key={problem.id}>
        <span className="angle-estimation-index">{index + 1}</span>
        <div className="angle-diagram" role="img" aria-label={`${index + 1}번 각도 그림`}>
          <span className="angle-ray angle-ray-base" />
          <span className="angle-ray angle-ray-turn" style={{ transform: `rotate(-${problem.angle}deg)` }} />
          <span className="angle-vertex" />
        </div>
        <div className="angle-answer-wrap">
          {answerSheet
            ? <strong className="angle-static-answer">{problem.angle}</strong>
            : <input
              className="angle-answer-input"
              data-angle-answer
              type="text"
              inputMode="numeric"
              pattern="[0-9]*"
              ...
              aria-label={`${index + 1}번 각도 답`}
            />}
          <span className="angle-degree">°</span>
        </div>
  ...
  function renderSheet(answerSheet: boolean) {
    return (
      <div className="a4-sheet counting-sheet angle-estimation-sheet" style={{ transform: `scale(${sheetScale})` }}>
  ...
  return (
    <main className="counting-page multiplication-page">
      ...
      <div className="a4-stage counting-a4-stage worksheet-stage" style={{ width: 794 * sheetScale, height: 1123 * sheetScale }} aria-label="A4 4학년 각도 어림 문제지">{renderSheet(false)}</div>
      <div className="a4-stage counting-a4-stage answer-stage" style={{ width: 794 * sheetScale, height: 1123 * sheetScale }} aria-label="A4 4학년 각도 어림 전체 답지">{renderSheet(true)}</div>
    </main>
  );
}
