...
export default function TranscendentalIntegralsPage() {
  ...
  function row(problem: TranscendentalIntegralProblem, index: number, answerSheet: boolean) {
    return (
      <article className="polynomial-question derivative-question trig-derivative-question" key={problem.id} data-testid="transcendental-integral-question">
    ...
    return (
      <div className={`a4-sheet counting-sheet polynomial-sheet derivative-sheet trig-derivative-sheet polynomial-sheet-${problems.length}`} style={{ transform: `scale(${scale})` }}>
        <header className="counting-sheet-header polynomial-sheet-header">
          <div className="counting-sheet-title"><span>미적분Ⅱ</span><strong>여러 함수의 부정적분{answerSheet ? " 정답" : ""}</strong></div>
    ...
    <main className="counting-page polynomial-page derivative-page trig-derivative-page">
      <div className="counting-toolbar">
        ...
        <div className="toolbar">
          ...
          {reviews.length === 0 && wrong.length > 0 && <button className="button secondary" onClick={() => setReviews(createTranscendentalIntegralReviewProblems(wrong.map(({ kind }) => kind), (set.seed ^ 0x9e3779b9) >>> 0))}>틀린 유형 {Math.min(wrong.length, 2)}문제 +</button>}
      ...
      {panelOpen && <WorksheetChoicePanel title="여러 함수의 부정적분" problems={choiceProblems} selected={selected} results={results} onSelect={(problemId, choiceId) => { setSelected((current) => ({ ...current, [problemId]: choiceId })); setResults((current) => { const next = { ...current }; delete next[problemId]; return next; }); }} onGrade={checkAll} onClose={() => setPanelOpen(false)} />}
      <div className="a4-stage counting-a4-stage worksheet-stage" style={{ width: 794 * scale, height: 1123 * scale }} aria-label="A4 여러 함수의 부정적분 문제지">{sheet(false)}</div>
      <div className="a4-stage counting-a4-stage answer-stage" style={{ width: 794 * scale, height: 1123 * scale }} aria-label="A4 여러 함수의 부정적분 정답지">{sheet(true)}</div>
    </main>
  );
}
