...
export default function ExponentialLogDerivativesPage() {
  ...
  function row(problem: ExponentialLogDerivativeProblem, index: number, answerSheet: boolean) {
    return (
      <article className="polynomial-question derivative-question trig-derivative-question" key={problem.id} data-testid="exponential-log-derivative-question">
    ...
    return (
      <div className={`a4-sheet counting-sheet polynomial-sheet derivative-sheet trig-derivative-sheet polynomial-sheet-${problems.length}`} style={{ transform: `scale(${scale})` }}>
        <header className="counting-sheet-header polynomial-sheet-header">
          <div className="counting-sheet-title"><span>미적분Ⅱ</span><strong>지수·로그함수 미분{answerSheet ? " 정답" : ""}</strong></div>
    ...
    <main className="counting-page polynomial-page derivative-page trig-derivative-page">
      <div className="counting-toolbar">
        ...
        <div className="toolbar">
          <button className="button secondary" onClick={() => { setSet(createExponentialLogDerivativeProblemSet(Date.now() >>> 0)); reset(); }}>새 문제</button>
          <button className="button ghost" onClick={reset}>다시 풀기</button>
          {reviews.length === 0 && wrong.length > 0 && <button className="button secondary" onClick={() => setReviews(createExponentialLogDerivativeReviewProblems(wrong.map(({ kind }) => kind), (set.seed ^ 0x9e3779b9) >>> 0))}>틀린 유형 {Math.min(wrong.length, 2)}문제 +</button>}
