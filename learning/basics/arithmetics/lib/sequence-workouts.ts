...
function build(kind:SequenceKind,next:()=>number,id:string):SequenceProblem{
 ...
 if(kind==="arithmetic-difference"){const a=int(next,-5,8),d=int(next,2,7),p=int(next,2,5),q=p+int(next,3,6);return{id,kind,label:"공차 구하기",prompt:"공차를 구하세요.",latex:`a_{${p}}=${a+(p-1)*d},\\quad a_{${q}}=${a+(q-1)*d},\\quad d=?`,answer:d}}
