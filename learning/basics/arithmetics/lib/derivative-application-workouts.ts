...
function build(kind:DerivativeApplicationKind,next:()=>number,id:string):DerivativeApplicationProblem{
  if(kind==="tangent-line"){
    ...
    return{id,kind,label:"접선의 방정식",prompt:"주어진 점에서의 접선을 𝑦=𝑚𝑥+𝑏로 나타낼 때 𝑚, 𝑏를 구하세요.",latex:`f(x)=x^3${signedTerm(p,"x^2")}${signedTerm(q,"x")},\\qquad x=${x}`,answerLabels:["m","b"],answers:[m,b]};
  }
