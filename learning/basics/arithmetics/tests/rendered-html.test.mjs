...
test("renders the learning index, arithmetic mode choice, and catalog in workbook order", async () => {
  ...
  assert.equal((indexHtml.match(/data-testid="worksheet-choice"/g) ?? []).length, 134);
  assert.equal((indexHtml.match(/class="worksheet-grade"/g) ?? []).length, 134);
  ...
  assert.equal((catalogHtml.match(/data-testid="worksheet-choice"/g) ?? []).length, 134);
  ...
  assert.equal((catalogHtml.match(/class="worksheet-grade"/g) ?? []).length, 134);
...
test("renders the unified arithmetic catalog and high-school worksheets", async () => {
  ...
  assert.equal((hubHtml.match(/data-testid="worksheet-choice"/g) ?? []).length, 134);
